...
class SimpleSwitch(app_manager.RyuApp):
	...
	self.logger.info("packet in %s %s %s %s", dpid, src, dst, msg.in_port)
		...
		if dst_ip[:8] == PUBLIC_IP[:8] and msg.in_port != 3:
			...
			if msg.in_port == 1:
                match = datapath.ofproto_parser.OFPMatch(dl_type = 0x0800,in_port = 1,nw_proto = ipPkt.proto,tp_src = src_port,
                                                            nw_src = src_ip,nw_dst_mask = 24,nw_dst = PUBLIC_IP)    
			else:
		        match = datapath.ofproto_parser.OFPMatch(dl_type = 0x0800,in_port = 2,nw_proto = ipPkt.proto,
                                                            nw_src = src_ip,nw_dst_mask = 24,nw_dst = PUBLIC_IP,tp_src = src_port)
			actions = [datapath.ofproto_parser.OFPActionSetDlSrc(R1_MAC_PUBLIC),
                        datapath.ofproto_parser.OFPActionSetDlDst(PUBLIC_MAC),
                        datapath.ofproto_parser.OFPActionSetNwSrc(R1_IP_PUBLIC),
                        datapath.ofproto_parser.OFPActionSetTpSrc(new_port),
			...
			if msg.in_port == 1:
				if dst_ip[:10] == LAN1[:10]:
					...
					actions = [datapath.ofproto_parser.OFPActionSetDlSrc(R1_MAC_LEFT),
                                datapath.ofproto_parser.OFPActionSetDlDst(BCAST),
				...
				if dst_ip[:10] == LAN2[:10]:
					...
					actions = [datapath.ofproto_parser.OFPActionSetDlSrc(R1_MAC_RIGHT),
                                datapath.ofproto_parser.OFPActionSetDlDst(R2_MAC_LEFT),
					...
					if dst_ip[:10] == LAN1[:10]:
						out_port = 2
						srcMAC = R1_MAC_LEFT
						match = datapath.ofproto_parser.OFPMatch(dl_type = 0x0800,in_port = 3,nw_proto = ipPkt.proto,nw_dst_mask = 24,
                                                                    nw_dst = PUBLIC_IP,tp_dst = dst_port)
						actions = [datapath.ofproto_parser.OFPActionSetDlSrc(srcMAC),
                                    datapath.ofproto_parser.OFPActionSetDlDst(BCAST),
                                    datapath.ofproto_parser.OFPActionSetTpDst(port),
                                    datapath.ofproto_parser.OFPActionSetNwDst(dst_ip),
                                    datapath.ofproto_parser.OFPActionOutput(out_port)]
					elif dst_ip[:10] == LAN2[:10]:
						out_port = 1
						srcMAC = R1_MAC_RIGHT
						match = datapath.ofproto_parser.OFPMatch(dl_type = 0x0800,in_port = 3,nw_proto = ipPkt.proto,
                                                                    nw_dst_mask = 24,nw_dst = PUBLIC_IP,tp_dst = dst_port)
						actions = [datapath.ofproto_parser.OFPActionSetDlSrc(srcMAC),
                                    datapath.ofproto_parser.OFPActionSetDlDst(R2_MAC_LEFT),
                                    datapath.ofproto_parser.OFPActionSetTpDst(port),
                                    datapath.ofproto_parser.OFPActionSetNwDst(dst_ip),
		...
		if msg.in_port == 1:
			...
			actions = [datapath.ofproto_parser.OFPActionSetDlSrc(R2_MAC_RIGHT),
                        datapath.ofproto_parser.OFPActionSetDlDst(BCAST),
                        datapath.ofproto_parser.OFPActionOutput(2)]
		elif msg.in_port == 2:
			ipPkt = pkt.get_protocol(ipv4.ipv4)
			...
			actions = [datapath.ofproto_parser.OFPActionSetDlSrc(R2_MAC_LEFT),
                        datapath.ofproto_parser.OFPActionSetDlDst(R1_MAC_RIGHT),
                        datapath.ofproto_parser.OFPActionOutput(1)]
